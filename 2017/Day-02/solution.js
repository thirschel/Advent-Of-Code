...
var checkSum = 0;
var evenlyDivisibleCheckSum = 0;
input.forEach(i=>{
  var highest = null;
  var lowest = null;
  i.forEach((num,idx)=>{
    highest = parseInt(num) > highest || highest === null ? parseInt(num) : highest;
    lowest = parseInt(num) < lowest || lowest === null ? parseInt(num) : lowest;
    i.forEach((jNum, jdx)=>{
      evenlyDivisibleCheckSum += jdx !== idx && (parseInt(num) / parseInt(jNum)) % 1 === 0 ? parseInt(num) / parseInt(jNum) : 0;
    })
  });
  checkSum += highest - lowest;
})

console.log(`First Part: ${checkSum}`);
console.log(`Second Part: ${evenlyDivisibleCheckSum}`);