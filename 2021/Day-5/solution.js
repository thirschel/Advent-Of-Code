...
var findOverlappingLinesWithDiagonals = function (coordinateInput) {
    ...
    coordinates.forEach(function (coords) {
        if (coords.x1 === coords.x2) {
            ...
            var xMovement = coords.x1 < coords.x2 ? 1 : -1;
            var yMovement = coords.y1 < coords.y2 ? 1 : -1;
            for (var i = 0; i <= Math.abs(coords.x1 - coords.x2); i++) {
                subMap[coords.y1 + (yMovement * i)][coords.x1 + (xMovement * i)]++;
                if (subMap[coords.y1 + (yMovement * i)][coords.x1 + (xMovement * i)] > 1) {
                    overlapCount.add("".concat(coords.y1 + (yMovement * i), ",").concat(coords.x1 + (xMovement * i)));
                }
            }
        }
    });
...
var parseCoordinates = function (coordinates) {
    var regex = /(\d+),(\d+) -> (\d+),(\d+)/;
    var parsed = coordinates.match(regex);
    return { x1: parseInt(parsed[1]), y1: parseInt(parsed[2]), x2: parseInt(parsed[3]), y2: parseInt(parsed[4]) };
};
var firstPart = "First part: ".concat(findOverlappingLines(input));
var secondPart = "Second part: ".concat(findOverlappingLinesWithDiagonals(input));
