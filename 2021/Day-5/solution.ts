...
const findOverlappingLines = (coordinateInput: Array<string>) => {
    var coordinates = coordinateInput.map(x => parseCoordinates(x));
    var subMap = Array(1000).fill(0).map(function () { return Array(1000).fill(0); });
    var overlapCount = new Set<string>();
...
const findOverlappingLinesWithDiagonals = (coordinateInput: Array<string>) => {
    var coordinates = coordinateInput.map(x => parseCoordinates(x));
    var subMap = Array(1000).fill(0).map(function () { return Array(1000).fill(0); });
    var overlapCount = new Set<string>();
    coordinates.forEach(coords => {
        if(coords.x1 === coords.x2) {
            ...
            for(var i = 0; i <= Math.abs(coords.x1 - coords.x2); i++) {
                ...
                if(subMap[coords.y1 + (yMovement * i)][coords.x1 + (xMovement * i)] > 1) {
                    overlapCount.add(`${coords.y1 + (yMovement * i)},${coords.x1 + (xMovement * i)}`);
                }
            }
        }
    });
...
const parseCoordinates = (coordinates: string): HydrothermalCoords => {
    var regex  = /(\d+),(\d+) -> (\d+),(\d+)/;
    const parsed = coordinates.match(regex);
    return { x1: parseInt(parsed[1]), y1: parseInt(parsed[2]), x2: parseInt(parsed[3]), y2: parseInt(parsed[4])} as HydrothermalCoords
}


const firstPart = `First part: ${findOverlappingLines(input)}` ;
const secondPart = `Second part: ${findOverlappingLinesWithDiagonals(input)}` ;
