...
const findUniqueOutputSignals = (codedSignals: Array<string>): number => {
    var uniqueSignalCount = 0;
    codedSignals.forEach(singalString => {
        var signal = parseInput(singalString);
        uniqueSignalCount += signal.outputs.reduce((a,b) => {
            if(b.length === 7 || b.length ===  2 || b.length === 3 || b.length === 4) {
                return a + 1
            }
            return a;
        }, 0);
    });
    return uniqueSignalCount;
}

const findOutputSum = (codedSignals: Array<string>): number => {
    var output = 0;
    codedSignals.forEach(singalString => {
        var signalMappings = new Map();
        var segmentMappings = new Map();
        var signal = parseInput(singalString);

        // One
        var oneMapping = signal.inputs.find(x => x.length === 2);
        var mappingSplit = oneMapping.split('');
        segmentMappings.set('c', mappingSplit[0]);
        segmentMappings.set('f', mappingSplit[1]);
        signalMappings.set(1, mappingSplit.sort().join(''));

        // Four
        var fourMapping = signal.inputs.find(x => x.length === 4);
        var mappingSplit = fourMapping.split('').filter(x => !Array.from(segmentMappings.values()).includes(x));
        var sixSignalNumbers = signal.inputs.filter(x => x.length === 6);
        if(sixSignalNumbers.filter(x => x.includes(mappingSplit[0])).length === 3) {
            segmentMappings.set('b', mappingSplit[0]);
            segmentMappings.set('d', mappingSplit[1]);
        }
        else {
            segmentMappings.set('d', mappingSplit[0]);
            segmentMappings.set('b', mappingSplit[1]);
        }
        signalMappings.set(4, fourMapping.split('').sort().join(''));

        // Seven
        var sevenMapping = signal.inputs.find(x => x.length === 3);
        var mappingSplit = sevenMapping.split('').filter(x => !Array.from(segmentMappings.values()).includes(x));
        segmentMappings.set('a',mappingSplit[0]);
        signalMappings.set(7, sevenMapping.split('').sort().join(''));

        // Eight
        var eightMapping = signal.inputs.find(x => x.length === 7);
        signalMappings.set(8, eightMapping.split('').sort().join(''));

        // Zero
        var zeroMapping = signal.inputs.find(x => x.length === 6 && !x.includes(segmentMappings.get('d')));
        signalMappings.set(0, zeroMapping.split('').sort().join('')); 

        // Six
        var sixMapping = signal.inputs.find(x => x.length === 6 && !x.includes(segmentMappings.get('c')));
        // The c and the f are flipped in this case
        if(!sixMapping) {
            var tempF = segmentMappings.get('f');
            segmentMappings.set('f', segmentMappings.get('c'));
            segmentMappings.set('c', tempF);
            sixMapping = signal.inputs.find(x => x.length === 6 && !x.includes(segmentMappings.get('c')));
        }
        signalMappings.set(6, sixMapping.split('').sort().join('')); 

        // Nine
        var nineMapping = signal.inputs.find(x => x.length === 6 && ![signalMappings.get(6), signalMappings.get(0)].includes(x.split('').sort().join('')));
        signalMappings.set(9, nineMapping.split('').sort().join('')); 
        var eMapping = [...signalMappings.get(6).split('')].filter(x => [...nineMapping.split('')].indexOf(x) == -1 && x !== segmentMappings.get('c'));
        segmentMappings.set('e', eMapping[0]);

        // Five
        var fiveMapping = signal.inputs.find(x => x.length === 5 && x.includes(segmentMappings.get('b')));
        signalMappings.set(5, fiveMapping.split('').sort().join('')); 

        // Three
        var threeMapping = signal.inputs.find(x => x.length === 5 && x.includes(segmentMappings.get('c')) && x.includes(segmentMappings.get('f')));
        signalMappings.set(3, threeMapping.split('').sort().join('')); 

        // Two
        var twoMapping = signal.inputs.find(x => x.length === 5 && x.includes(segmentMappings.get('e')));
        signalMappings.set(2, twoMapping.split('').sort().join('')); 
        
        var signalMappingsArr = Array.from(signalMappings.entries());
        var signalOutput = signal.outputs.reduce((a,b) =>  a + signalMappingsArr.find(x => x[1] === b.split('').sort().join(''))[0],'') 
        output += parseInt(signalOutput)
    });
    return output;
}

const parseInput = (inputString: string) => {
    var singalSplit = inputString.split(' | ');
    return {inputs: singalSplit[0].split(' '), outputs: singalSplit[1].split(' ')}
}

const firstPart = `First part: ${findUniqueOutputSignals(input)}` ;
const secondPart = `Second part: ${findOutputSum(input)}` ;
