    ...
    for (var i = 5; i < balls.length; i++) {
        ...
        for (var x = 0; x < bingoCards.length; x++) {
            for (var y = 0; y < bingoCards[x].winningCombos.length; y++) {
                if (Array.from(bingoCards[x].winningCombos[y].keys()).reduce(function (a, b) { return calledBalls.has(b) ? a : false; }, true)) {
                    return bingoCards[x].board.map(function (board) { return board.reduce(function (a, b) { return calledBalls.has(b) ? a : a + b; }, 0); }).reduce(function (a, b) { return a + b; }, 0) * balls[i];
                }
            }
        }
    }
};
var findLastBingo = function (balls, cards) {
    var bingoCards = cards.map(function (x) { return parseBingoBoard(x); });
    var calledBalls = new Set(__spreadArray([], balls, true).slice(0, 5));
    for (var i = 5; i < balls.length; i++) {
        calledBalls.add(balls[i]);
        var nonWinningBingoCards = Array();
        for (var x = 0; x < bingoCards.length; x++) {
...
var parseBingoBoard = function (board) {
    var winningCombos = [];
    var winningDiagonal = new Set();
    var winningRows = [];
    for (var x = 0; x < board.length; x++) {
        var winningColumn = new Set();
        for (var y = 0; y < board[x].length; y++) {
            ...
            if (winningRows.length <= y && y !== board[x].length) {
                winningRows.push(new Set([board[x][y]]));
            }
            else {
    ...
    winningCombos.push.apply(winningCombos, winningRows);
    return { board: board, winningCombos: winningCombos };
};
var firstPart = "First part: ".concat(findFirstBingo(bingoBallsInput, bingoBoardInput));
var secondPart = "Second part: ".concat(findLastBingo(bingoBallsInput, bingoBoardInput));
