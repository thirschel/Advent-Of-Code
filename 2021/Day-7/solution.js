...
var getConstantRateCrabMovement = function (testinput) {
    var min = Math.min.apply(Math, testinput);
    var max = Math.max.apply(Math, testinput);
    var crabPositions = new Map();
    for (var i = min; i < max; i++) {
        crabPositions.set(i, __spreadArray([], testinput, true).reduce(function (a, b) { return a + Math.abs(b - i); }, 0));
    }
    return Math.min.apply(Math, Array.from(crabPositions.values()));
};
var getDynamicRateCrabMovement = function (testinput) {
    var min = Math.min.apply(Math, testinput);
    var max = Math.max.apply(Math, testinput);
    var crabPositions = new Map();
    for (var i = min; i < max; i++) {
        crabPositions.set(i, __spreadArray([], testinput, true).reduce(function (a, b) {
            var n = Math.abs(b - i);
            return a + (n * (n + 1) / 2);
        }, 0));
    }
...
var firstPart = "First part: ".concat(getConstantRateCrabMovement(input));
var secondPart = "Second part: ".concat(getDynamicRateCrabMovement(input));
