...
const calculateTotalFuelRequirement = (modules: number[]) => {
    let totalRequirment = 0;
    modules.forEach(module => {
        let fuel = (Math.floor(module / 3)) - 2;
        totalRequirment += fuel;
...
const firstPart = `First part: ${calculateFuelRequirement(input)}` ;
const secondPart = `Second part: ${calculateTotalFuelRequirement(input)}`;
