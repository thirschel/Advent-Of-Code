...
class LocationInformation {
    public wire1Steps: number;
    public wire2Steps: number;
...
const traceWire = (wireCoords: string[], wireMarker: number) => {
    let xPos = 0;
    let yPos = 0;
    var stepsTaken = 0;
    wireCoords.forEach(coords => {
        const matches = coords.match(/(\w)(\d+)/);
        const direction = matches[1] as Direction;
        const distance = Number(matches[2]);

        let xChangePerDistance = 0;
        let yChangePerDistance = 0;
        switch(direction) {
            case Direction.Left:
        ...
        for(var i = 0; i < distance; i++) {
            ...
            let locationInfo = visitedCoords.get(`${yPos},${xPos}`) as LocationInformation;
            ...
            if(!locationInfo){
                ...
                locationInfo.wire2Steps = locationInfo.wire2Steps ?? stepsTaken;
                locationInfo.isIntersection = true;
            }

            else if(locationInfo && (wireMarker === 1 && locationInfo.wire2Steps)) {
                locationInfo.wire1Steps = locationInfo.wire1Steps ?? stepsTaken;
                locationInfo.isIntersection = true;
            }
...
const findIntersections = (wires: string[][]) => {
    wires.forEach((wire, i) => {
        traceWire(wire, i + 1);
    })
    return Array.from(visitedCoords.entries()).filter(c => c[1].isIntersection);
}

const intersections = findIntersections(input);

const firstPartAnswer = Math.min(...intersections.map(c => {
    const matches = c[0].match(/(-?\d+),(-?\d+)/);
    return Math.abs(Number(matches[1]))+Math.abs(Number(matches[2]))
}));
const secondPartAnswer = Math.min(...intersections.map(c => {
    return Math.abs(c[1].wire1Steps)+Math.abs(c[1].wire2Steps)
}));



const firstPart = `First part: ${firstPartAnswer}` ;
const secondPart = `Second part: ${secondPartAnswer}`;
