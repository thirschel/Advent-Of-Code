...
var LocationInformation = /** @class */ (function () {
...
var visitedCoords = new Map();
var traceWire = function (wireCoords, wireMarker) {
    var xPos = 0;
    var yPos = 0;
    var stepsTaken = 0;
    wireCoords.forEach(function (coords) {
        var _a, _b;
        var matches = coords.match(/(\w)(\d+)/);
        var direction = matches[1];
        var distance = Number(matches[2]);
        var xChangePerDistance = 0;
        var yChangePerDistance = 0;
        switch (direction) {
            case Direction.Left:
                xChangePerDistance = -1;
                break;
            case Direction.Right:
                xChangePerDistance = 1;
                break;
            case Direction.Up:
                yChangePerDistance = -1;
                break;
            case Direction.Down:
                yChangePerDistance = 1;
                break;
        ...
        for (var i = 0; i < distance; i++) {
            stepsTaken++;
            yPos += yChangePerDistance;
            xPos += xChangePerDistance;
            var locationInfo = visitedCoords.get(yPos + "," + xPos);
            // Location has not been visited by either wire yet
            if (!locationInfo) {
                locationInfo = new LocationInformation();
                if (wireMarker === 1) {
                    locationInfo.wire1Steps = stepsTaken;
                }
                else {
                    locationInfo.wire2Steps = stepsTaken;
                }
            }
            else if (locationInfo && (wireMarker === 2 && locationInfo.wire1Steps)) {
                locationInfo.wire2Steps = (_a = locationInfo.wire2Steps, (_a !== null && _a !== void 0 ? _a : stepsTaken));
                locationInfo.isIntersection = true;
            }
            else if (locationInfo && (wireMarker === 1 && locationInfo.wire2Steps)) {
                locationInfo.wire1Steps = (_b = locationInfo.wire1Steps, (_b !== null && _b !== void 0 ? _b : stepsTaken));
                locationInfo.isIntersection = true;
            }
            visitedCoords.set(yPos + "," + xPos, locationInfo);
        }
    });
};
var findIntersections = function (wires) {
    wires.forEach(function (wire, i) {
        traceWire(wire, i + 1);
    });
    return Array.from(visitedCoords.entries()).filter(function (c) { return c[1].isIntersection; });
};
var intersections = findIntersections(input);
var firstPartAnswer = Math.min.apply(Math, intersections.map(function (c) {
    var matches = c[0].match(/(-?\d+),(-?\d+)/);
    return Math.abs(Number(matches[1])) + Math.abs(Number(matches[2]));
}));
var secondPartAnswer = Math.min.apply(Math, intersections.map(function (c) {
    return Math.abs(c[1].wire1Steps) + Math.abs(c[1].wire2Steps);
}));
var firstPart = "First part: " + firstPartAnswer;
var secondPart = "Second part: " + secondPartAnswer;
