...
const getWireValue =wireKey =>{
  const wire = wires.get(wireKey);
  ...
  if (!wire.command) {
    wires.set(wireKey, getWireValue(wire.args[0]));
  } else {
    wires.set(wireKey, BITWISE_METHODS[wire.command](getWireValue(wire.args[0]), getWireValue(wire.args[1])));
  } 
 
  return wires.get(wireKey);
}

var wires = new Map();
input.forEach(i=>{
  const cleanInstructions = parseInstruction(i);
  wires.set(cleanInstructions.destination, {command: cleanInstructions.command, args: cleanInstructions.args});
});
console.log('Part 1: '+ getWireValue('a'));
...
wires.set('b', 956);  
console.log('Part 2: '+ getWireValue('a'));