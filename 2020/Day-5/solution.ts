...
const _TOTAL_ROWS = 128;
const _TOTAL_COLUMNS = 8;

const scanBoardingPasses = (rawPasses: Array<string>) => {
    const boardingPasses: Array<BoardingPass> = [];
    rawPasses.forEach(pass => {
        let rowBottom = 0;
        let columnBottom = 0;
        // 0 based
        let rowTop = _TOTAL_ROWS - 1;
        // 0 based
        let columnTop = _TOTAL_COLUMNS - 1;
        pass.split('').forEach(positionCharacter => {
...
const findMissingBoardingPass = (boardingPasses:  Array<BoardingPass>) => {
    const seats: string[][] = Array(_TOTAL_ROWS).fill('').map(() => Array(_TOTAL_COLUMNS).fill('O'));
    boardingPasses.forEach(pass => seats[pass.row][pass.column] = 'X');
    for(var y = 0; y < seats.length; y++) {
        for(var x = 0; x < seats[0].length; x++) {
            if(seats[y][x] === 'O') {
                var missingBoardingPass = new BoardingPass(y, x);
                if(boardingPasses.some(p => p.seatId === missingBoardingPass.seatId - 1) &&
                   boardingPasses.some(p => p.seatId === missingBoardingPass.seatId + 1))
...
const boardingPasses = scanBoardingPasses(input);

const firstPart = `First part: ${Math.max(...boardingPasses.map(b => b.seatId))}` ;
const secondPart = `Second part: ${findMissingBoardingPass(boardingPasses).seatId}`;
