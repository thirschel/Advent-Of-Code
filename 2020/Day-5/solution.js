...
var _TOTAL_ROWS = 128;
var _TOTAL_COLUMNS = 8;
var scanBoardingPasses = function (rawPasses) {
    var boardingPasses = [];
    rawPasses.forEach(function (pass) {
        var rowBottom = 0;
        var columnBottom = 0;
        // 0 based
        var rowTop = _TOTAL_ROWS - 1;
        // 0 based
        var columnTop = _TOTAL_COLUMNS - 1;
        pass.split('').forEach(function (positionCharacter) {
            switch (positionCharacter) {
                case 'F':
                    rowTop = rowTop - (Math.floor((rowTop - rowBottom) / 2)) - 1;
                    break;
                case 'B':
                    rowBottom = rowBottom + (Math.ceil((rowTop - rowBottom) / 2));
                    break;
                case 'L':
                    columnTop = columnTop - (Math.floor((columnTop - columnBottom) / 2)) - 1;
                    break;
                case 'R':
                    columnBottom = columnBottom + (Math.ceil((columnTop - columnBottom) / 2));
                    break;
            }
        });
        boardingPasses.push(new BoardingPass(rowBottom, columnBottom));
    });
    return boardingPasses;
};
var findMissingBoardingPass = function (boardingPasses) {
    var seats = Array(_TOTAL_ROWS).fill('').map(function () { return Array(_TOTAL_COLUMNS).fill('O'); });
    boardingPasses.forEach(function (pass) { return seats[pass.row][pass.column] = 'X'; });
    for (var y = 0; y < seats.length; y++) {
        for (var x = 0; x < seats[0].length; x++) {
            if (seats[y][x] === 'O') {
                var missingBoardingPass = new BoardingPass(y, x);
                if (boardingPasses.some(function (p) { return p.seatId === missingBoardingPass.seatId - 1; }) &&
                    boardingPasses.some(function (p) { return p.seatId === missingBoardingPass.seatId + 1; }))
                    return missingBoardingPass;
            }
        }
    }
};
var boardingPasses = scanBoardingPasses(input);
var firstPart = "First part: " + Math.max.apply(Math, boardingPasses.map(function (b) { return b.seatId; }));
var secondPart = "Second part: " + findMissingBoardingPass(boardingPasses).seatId;
