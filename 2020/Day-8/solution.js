...
var firstRunAccumulator;
var parseInput = function (rawCommand) {
    var regex = /(nop|acc|jmp) (\+?-?\d+)/;
    var matches = rawCommand.match(regex);
    var command = new Command();
    command.argument = parseInt(matches[2]);
    if (matches[1] === 'nop') {
        command.operation = Operation.nop;
    }
    else if (matches[1] === 'acc') {
        command.operation = Operation.acc;
    }
    else {
...
var bootCode = input.map(function (i) { return parseInput(i); });
var runGame = function (bootCode) {
    var runLinesOfCode = new Set();
    var accumulator = 0;
    for (var i = 0; i < bootCode.length; i) {
        if (runLinesOfCode.has(i)) {
            throw "" + accumulator;
        }
        runLinesOfCode.add(i);
        switch (bootCode[i].operation) {
            case Operation.acc:
                accumulator += bootCode[i].argument;
                i++;
                break;
            case Operation.jmp:
                i += bootCode[i].argument;
                break;
            case Operation.nop:
                i++;
                break;
...
var repairBootCode = function (bootCode) {
    for (var i = 0; i < bootCode.length; i++) {
        var newCommands = JSON.parse(JSON.stringify(bootCode));
        try {
            if (newCommands[i].operation === Operation.jmp) {
                newCommands[i].operation = Operation.nop;
                return runGame(newCommands);
            }
            else if (newCommands[i].operation === Operation.nop) {
                newCommands[i].operation = Operation.jmp;
                return runGame(newCommands);
            }
        }
        catch (_a) { }
...
try {
    ...
    firstRunAccumulator = exception;
}
var firstPart = "First part: " + firstRunAccumulator;
var secondPart = "Second part: " + repairBootCode(bootCode);
