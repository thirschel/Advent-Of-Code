...
let firstRunAccumulator;


const parseInput = (rawCommand:string) => {
    const regex = /(nop|acc|jmp) (\+?-?\d+)/;
...
const bootCode = input.map(i => parseInput(i));

const runGame = (bootCode: Array<Command>) => {
    const runLinesOfCode = new Set<number>();
    let accumulator = 0;
...
const repairBootCode = (bootCode: Array<Command>) => {
    for(var i = 0; i < bootCode.length; i++) {
        const newCommands = JSON.parse(JSON.stringify(bootCode)) as Array<Command>;
        try {
...
const firstPart = `First part: ${firstRunAccumulator}` ;
const secondPart = `Second part: ${repairBootCode(bootCode)}`;
