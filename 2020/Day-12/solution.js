...
var Direction = /** @class */ (function () {
    function Direction(heading, units) {
        this.heading = heading;
        this.units = units;
    }
    return Direction;
}());
var parseInput = function (direction) {
    var regex = /(\w)(\d+)/;
    var matches = direction.match(regex);
    return new Direction(matches[1], parseInt(matches[2]));
};
var findNewShipDirection = function (shipFacingDirection, direction) {
    var headings = direction.heading === Heading.Left ?
        [Heading.East, Heading.North, Heading.West, Heading.South] :
        [Heading.East, Heading.South, Heading.West, Heading.North];
    var currentHeadingIndex = headings.indexOf(shipFacingDirection);
    var turnAmounts = direction.units / 90;
    return headings[(currentHeadingIndex + turnAmounts) % headings.length];
};
var findNewWaypointLocation = function (waypointX, waypointY, direction) {
    var turnAmounts = (direction.units / 90) % 4;
    var newWaypoint = { x: waypointX, y: waypointY };
    for (var i = 0; i < turnAmounts; i++) {
...
var navigateShip = function (directions) {
    var shipFacingDirection = Heading.East;
    var positionX = 0;
    var positionY = 0;
    directions.forEach(function (rawDirection) {
        var direction = parseInput(rawDirection);
...
var navigateShipWithWaypoint = function (directions) {
    var shipX = 0;
    var shipY = 0;
    var waypointX = 10;
    var waypointY = -1;
    directions.forEach(function (rawDirection) {
        ...
        switch (direction.heading) {
            ...
            case Heading.Left:
            case Heading.Right:
                var newWaypoints = findNewWaypointLocation(waypointX, waypointY, direction);
...
var firstPart = "First part: " + navigateShip(input);
var secondPart = "Second part: " + navigateShipWithWaypoint(input);
