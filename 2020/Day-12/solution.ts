...
class Direction {
    heading: Heading;
    units: number;
    constructor(heading: Heading, units: number) {
        this.heading = heading;
        this.units = units;
    }
}

const parseInput = (direction:string) => {
    var regex = /(\w)(\d+)/;
    const matches = direction.match(regex);
    return new Direction(matches[1] as Heading, parseInt(matches[2]));
}

const findNewShipDirection = (shipFacingDirection: Heading, direction: Direction) => {
    let headings = direction.heading === Heading.Left ? 
    [Heading.East, Heading.North, Heading.West, Heading.South] :
    [Heading.East, Heading.South, Heading.West, Heading.North];
    const currentHeadingIndex = headings.indexOf(shipFacingDirection);
    const turnAmounts = direction.units / 90;
    return headings[(currentHeadingIndex + turnAmounts) % headings.length];
}

const findNewWaypointLocation = (waypointX: number, waypointY: number, direction: Direction) : {x,y} => {
    const turnAmounts = (direction.units / 90) % 4;
    let newWaypoint = {x: waypointX, y:waypointY};
    for(let i = 0 ; i < turnAmounts ; i++){
        if(direction.heading === Heading.Left){
            newWaypoint = {x: newWaypoint.y, y: -newWaypoint.x};
        }
        else{
            newWaypoint = {x: -newWaypoint.y, y:newWaypoint.x};
        }
...
const navigateShip = (directions: Array<string>) => {
    let shipFacingDirection = Heading.East;
    let positionX = 0;
    let positionY = 0;
    directions.forEach(rawDirection => {
        const direction = parseInput(rawDirection);
        switch (direction.heading) {
            case Heading.North:
                positionY -= direction.units;
                break;
            case Heading.South:
                positionY += direction.units;
                break;
            case Heading.East:
                positionX += direction.units;
                break;
            case Heading.West:
                positionX -= direction.units;
                break;
            case Heading.Forward:
                if(shipFacingDirection === Heading.North) {
                    positionY -= direction.units;
                }
                else if(shipFacingDirection === Heading.South) {
                    positionY += direction.units;
                }
                else if(shipFacingDirection === Heading.East) {
                    positionX += direction.units;
                }
                else if(shipFacingDirection === Heading.West) {
                    positionX -= direction.units;
                }
                break;
            case Heading.Left:
            case Heading.Right:
                shipFacingDirection = findNewShipDirection(shipFacingDirection, direction);
                break;
        }
    })
    return Math.abs(positionY) + Math.abs(positionX);
}

const navigateShipWithWaypoint = (directions: Array<string>) => {
    let shipX = 0;
    let shipY = 0;
    let waypointX = 10;
    let waypointY = -1;
    directions.forEach(rawDirection => {
        ...
        switch (direction.heading) {
            case Heading.North:
                waypointY -= direction.units;
                break;
            case Heading.South:
                waypointY += direction.units;
                break;
            case Heading.East:
                waypointX += direction.units;
                break;
            case Heading.West:
                waypointX -= direction.units;
                break;
            case Heading.Forward:
                shipY += waypointY * direction.units;
                shipX += waypointX * direction.units;
                break;
            case Heading.Left:
            case Heading.Right:
                const newWaypoints = findNewWaypointLocation(waypointX, waypointY, direction);
                waypointX = newWaypoints.x;
                waypointY = newWaypoints.y;
                break;
        }
    })
...
const firstPart = `First part: ${navigateShip(input)}` ;
const secondPart = `Second part: ${navigateShipWithWaypoint(input)}`;
