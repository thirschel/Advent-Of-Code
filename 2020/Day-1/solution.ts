...
const findThreeNumberExpenseIssue = (expenses: Array<number>) => {
    for(var i = 0; i < expenses.length; i++) {
        for(var j = 0; j < expenses.length; j++) {
...
const firstPart = `First part: ${findTwoNumberExpenseIssue(input)}` ;
const secondPart = `Second part: ${findThreeNumberExpenseIssue(input)}`;
